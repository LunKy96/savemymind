...
REQUIRED = ("package", "cve", "cpe")
OPTIONAL = ("sha1", "file_path", "author", "description")
DATA_FIELDS = REQUIRED + OPTIONAL
...
<aside id="drawer" aria-hidden="true">
  ...
  <form id="entry-form" autocomplete="off" style="display:contents">
    <div class="dbody">
      ...
      <div class="field"><label for="f-package">Package <span class="req"></span></label><input id="f-package" placeholder="pkg:npm/lodash@4.17.15" required></div>
      <div class="g2">
        ...
        <div class="field"><label for="f-cpe">CPE <span class="req"></span></label><input id="f-cpe" placeholder="cpe:2.3:a:...:*" required></div>
      </div>
      <div class="g2">
...
async function save(ev){
  ...
  for(const k of ["package","cve","cpe"]) if(!d[k].trim()){ toast("Package, CVE/GHSA and CPE are required",true); return; }
  ...
  box.innerHTML=items.map((e,i)=>{
    ...
    return `<div class="rec" style="--i:${i}">
      <div class="rmain">
        ...
        ${idcell("pkg","package",e.package)}
        <span class="t-cpe">${idcell("cpe","CPE",e.cpe)}</span>
...
document.addEventListener("keydown",e=>{ if(e.key==="Escape"){ closeDrawer(); $("#dupmodal").classList.remove("open"); $("#export-menu").classList.remove("open"); } });
